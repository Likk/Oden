...
describe 'about Oden::Command::Dictionary#run' => sub {
    ...
    context 'message prefix `rename`' => sub {
        # Negative testing
        context 'case message pattern is `rename` only. no key, no value' => sub {
            before all => sub {
        ...
        context 'case message pattern is `rename <key>` only. no value' => sub {
            before all => sub {
                my $receiver = Oden::Entity::CommunicationReceiver->new(
                    message  => 'rename foo',
                    guild_id => 1,
        ...
        # Positive testing
        context 'case message pattern is `rename <key> <value>`' => sub {
            before all => sub {
                my $receiver = Oden::Entity::CommunicationReceiver->new(
            ...
            context 'case success' => sub {
                before all => sub {
                    $hash->{mock}->{dict} = Oden::Model::Dictionary->stubs('move', sub {
                        return 1;
                    });
                };
